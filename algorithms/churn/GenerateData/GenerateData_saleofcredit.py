...
input1 = sq.read.parquet("/hadoop/hdfs/INPUTPARQUET/contr/Contract_parquet")
input1.registerTempTable("input1")

output= sq.sql("select * from input INNER JOIN (SELECT ContractID as cd FROM input1 INNER JOIN filter ON input1.ClientID = filter.ClientID) FOO on input.ContractID=FOO.cd")
