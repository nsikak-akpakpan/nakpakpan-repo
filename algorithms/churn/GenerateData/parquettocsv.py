...
#input1 = sq.read.parquet("/hadoop/hdfs/INPUTPARQUET/crb/CreditBureauRaport_parquet")
#input1.registerTempTable("input1")

#output= sq.sql("select * from input INNER JOIN (SELECT RaportID as cd FROM input1 INNER JOIN filter ON input1.ClientID = filter.ClientID) FOO on input.RaportID=FOO.cd")
...
output = input.repartition(1)

output.write.format("com.databricks.spark.csv").options(header='true').save(csvout)
