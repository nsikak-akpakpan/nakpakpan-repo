...
parser.add_argument('--out',            nargs = '?', default = "hdfs://10.82.187.10:8020/hadoop/hdfs/OUT/PricesForVisuals_Camp_26-42-43_bnd50_NIR_5-10_m7.5", help = "Path to output table without \'_parquet\'")
...
parser.add_argument('--minMargin',      nargs = '*', default = 0.0  , type = float, help = 'Minimum Margin')
...
minComs       = inP.minCom      
maxComs       = inP.maxCom      
minMargins    = inP.minMargin   
maxMargins    = inP.maxMargin   
NIRBandwidths = inP.NIRBandwidth
ComBandwidths = inP.ComBandwidth
...
params = []
for k in range(len(minNIRs)):
    params.extend([(minNIRs[k], maxNIRs[k], minComs[k], maxComs[k], minMargins[k], maxMargins[k], NIRBandwidths[k], ComBandwidths[k])])
...
for pp in params:
    ...
    n += 1
    if saveToHDFS:
        result.write.mode('overwrite').parquet(outPath + str(n) + "_parquet")
    else:
